...
$(function(){
	...
	$('#insertItMyBtn').click(function(){
		...
		var cdamount = $('#amount').val();
		var cdgram = $('#gram').val();
		var cdcal = $('#kcal').val();
		var cdtan = $('#tan').text().substr(0,$('#tan').text().indexOf('('));
		var cddan = $('#dan').text().substr(0,$('#dan').text().indexOf('('));
		var cdji = $('#ji').text().substr(0,$('#ji').text().indexOf('('));
		var cdsik = $('#sik').text().substr(0,$('#sik').text().indexOf('('));
		var cdna = $('#na').text().substr(0,$('#na').text().indexOf('('));
...
<div id="wrap2">
	...
	<div class="diary-content">
		<form id="MFsearchFrm">
		    <input type="text" name="searchWord" class="calTxt" placeholder="음식명 검색" value="${WORD}"/>
...
<form class="diary-div" action="./insertMyFood2.com" id="detailFrm" method="post">
   ...
   <div class="diary-content">
      ...
      <input type="hidden" name="mfname" id="detail-cdname"/>
